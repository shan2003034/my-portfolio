    ...
    >
      ...
      <div className="h-60 w-full bg-[#1a1a1a] relative overflow-hidden">
        {image ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-gray-700 font-bold text-2xl">Image Placeholder</div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-[#111] to-transparent"></div>
      </div>

      <div className="p-8 flex flex-col flex-grow">
        <h3 className="text-3xl font-bold text-white mb-3 group-hover:text-[#deff9a] transition-colors">{title}</h3>
        <p className="text-gray-400 font-light leading-relaxed mb-6 flex-grow">{description}</p>
        
        {/* Tech Stack Pills */}
        <div className="flex flex-wrap gap-2 mb-8">
          {techStack.map((tech, index) => (
            <span key={index} className="text-xs font-bold text-black bg-[#deff9a] px-3 py-1 rounded-full">
              {tech}
            </span>
          ))}
        </div>
